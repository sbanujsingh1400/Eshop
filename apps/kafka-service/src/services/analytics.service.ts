...
// always store product_view for recommendations
    ...
    } else if([ "add_to_wishlist",
    "add_to_cart"].includes(event.action) && !actionExists){
    ...
    //   keep only last 100 action  (prevent storage)
...
export const updateProductAnalytics = async (event:any)=>{
        try {
           ...
           if(event.action==='add_to_wishlist') updateFields.wishlistsAdds={increment:1}
           if(event.action==='remove_from_cart') updateFields.cartAdds={decrement:1}
           if(event.action==='remove_from_wishlist') updateFields.wishlistsAdds={decrement:1}
