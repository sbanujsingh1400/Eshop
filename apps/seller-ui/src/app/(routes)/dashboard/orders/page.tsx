...
const OrdersTable = () => {
  ...
  const columns = useMemo(() => [{ accessorKey: 'id', header: 'Order ID', cell: ({ row }: any) => (<span className='font-mono text-sm font-medium text-slate-300'>{row.original.id.slice(-6).toUpperCase()}</span>) }, {
    ...
    header: " Buyer",
    cell: ({ row }: any) => (<span className='font-medium text-slate-200'>{row.original.users?.name ?? "Guest"}</span>) 
  ...
  },
   {
    accessorKey: "total",
    header: "Total",
    cell: ({ row }: any) => <span className='font-semibold text-slate-100'>${row.original.total.toFixed(2)}</span>
  },
  {
    accessorKey: "deliveryStatus",
    header: "Status",
    cell: ({ row }: any) => <span className={`px-2.5 py-1 rounded-full text-xs font-semibold ${row.original.deliveryStatus === "Cancelled" ?"bg-slate-500/10 text-red-400" : "bg-yellow-500/10 text-yellow-400" }`} >{row.original.deliveryStatus}</span>
  },
  {
  ...
  return (
    ...
    <div className='w-full space-y-6' >
     ...
     <div className="flex justify-between items-center">
      <h2 className='text-3xl text-white font-bold' >All Orders</h2>
      </div> 
    ...
    <span className='text-slate-400' >All Orders</span>
</div>
{/* Search Bar */}
<div className="flex items-center bg-slate-800/80 p-2 rounded-lg border border-slate-700 focus-within:ring-2 focus-within:ring-blue-500 transition">
  ...
  <input
    type="text"
    placeholder="Search orders by ID, buyer, status..."
...
<div className="overflow-x-auto bg-slate-800/80 rounded-lg border border-slate-700">
  {isLoading ? (
    <p className="text-center py-20 text-slate-400">Loading orders...</p>
  ) : (
...
{!isLoading && orders.length ===0 && <p className='text-center py-20 text-slate-500' >No Orders found!</p>}
